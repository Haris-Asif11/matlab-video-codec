...
blockrows = size(input_image,1)/blocksize;
blockcolumns = size(input_image, 2)/blocksize;
...
for i = 1:blockrows
    ...
    for j = 1:blockcolumns
        motion_compensated_frame(blockstart_y:blockstart_y+blocksize-1, blockstart_x:blockstart_x+blocksize-1)= padded_reference(blockstart_y+motion_vectors(x,1):blockstart_y+blocksize+motion_vectors(x,1)-1, blockstart_x+motion_vectors(x,2):blockstart_x+blocksize+motion_vectors(x,2)-1);
