function [rate,psnr,enc_time,dec_time]=get_dr_result(encoder_function, decoder_function, sequence, qp_values, transform_size, me_searchrange,me_blocksize)
...
%
% comfort funciton using CIF format and 5 frames to 
...
for qp_count=1:length(qp_values)
  ...
  enc_cmd=sprintf('%s(sequence,coded_file,352,288,100,transform_size,qp,me_blocksize,me_searchrange);',encoder_function);
...
save(statfile,'psnr','rate','sequence','qp_values','encoder_function','decoder_function','transform_size','me_searchrange','me_blocksize');
